...
public class CarryingCapacity {
    ...
    public ArrayList<Entity> getTopCapacity() {
        ...
        topCapacity.clear();
        findCapacity(valueEntities);
        return topCapacity;
    }

    private void findCapacity(int ents) {
        if (valueEntities == 1 && entities[0].getWeight() <= backpack.getCapacity()) {
    ...
    private void swap(int values) {
        ...
        for (int i = 0; i < valueEntities; i++) {
            System.out.print(entities[i].toString() + " || " );
        }
    }

    private void check(){
        for (int j = 0; j < valueEntities; j++) {
            if (entities[j].getWeight() <= backpack.getFreeCapacity()){
